...
{
    argc=3; argv[1]="10.10.20.98"; argv[2]="9106";          //IP,PORT 번호 할당
    int sock;
    ...
    write(sock,"03",3); //지역코드 전송
    read(sock,message,BUF_SIZE); //연결 확인 메시지
    printf("%s \n",message);


    file=fopen("Gyeongbuk_2013.csv", "r");
    while(1)
    {
    ...
    file=fopen("Gyeongbuk_2014.csv", "r");
    while(1)
    {
        fgets(message, BUF_SIZE, file);
        if(feof(file)) break;
    ...
    while(1)
    {
        ...
        read(sock, item, 10);
        item[strlen(item)]=0;
        FILE* fp=fopen("Gyeongbuk_2022.csv","r");
        if(fp==NULL){
                puts("파일오픈 실패!");
                return NULL;
            }
